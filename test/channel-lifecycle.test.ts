...
/** Serve a channel whose actions are exactly these, in exactly this order. */
function serve(actions: Action[]): void {
  const transactions = actions.map((a) => ({
    transaction: { data: JSON.stringify({ data: a }) },
...
const read = () => fetchInvoice(REQUEST_ID, { gatewayUrl: "https://stub.invalid/" });
...
describe("the whole channel decides what is owed", () => {
  ...
  test("a cancel AFTER the create refuses", async () => {
    serve([CREATE, { name: "cancel" }]);
    assert.equal(await refusalCode(read), "INVOICE_CANCELLED");
  });
  ...
  test("an increase raises the amount by its delta, not to it", async () => {
    ...
    serve([CREATE, { name: "increaseExpectedAmount", parameters: { deltaAmount: "500000000000000000" } }]);
  ...
  test("a reduction lowers it, and the debt settles at the reduced figure", async () => {
    serve([CREATE, { name: "reduceExpectedAmount", parameters: { deltaAmount: "250000000000000000" } }]);
    assert.equal((await read()).invoiceBaseUnits, "750000000000000000");
  });
  ...
  test("a delta placed before the create refuses", async () => {
    ...
    // channel never had.
    serve([{ name: "increaseExpectedAmount", parameters: { deltaAmount: ONE } }, CREATE]);
    assert.equal(await refusalCode(read), "MALFORMED_TRANSACTION");
  });
  ...
  test("an action name this reader does not understand refuses", async () => {
    ...
    serve([CREATE, { name: "someFutureAmountAction", parameters: { deltaAmount: ONE } }]);
    assert.equal(await refusalCode(read), "MALFORMED_TRANSACTION");
  });
  ...
  test("an accept does not change the amount", async () => {
    ...
    serve([CREATE, { name: "accept" }]);
    assert.equal((await read()).invoiceBaseUnits, ONE);
  });
});
