...
export function toBaseUnits(human: string, decimals: number): bigint {
  ...
  if (!match) {
    throw new MoneyError(
      "NOT_DECIMAL",
      `not a plain non-negative decimal: ${JSON.stringify(human)}`,
    );
  }
  ...
  if (frac.length > decimals) {
    throw new MoneyError(
      "PRECISION_LOSS",
      `${human.trim()} has ${frac.length} decimal places but the token has ${decimals}`,
    );
  }
...
export function toHuman(base: bigint, decimals: number): string {
  assertDecimals(decimals);
  if (base < 0n) throw new MoneyError("NEGATIVE", `base units must be non-negative, got ${base}`);
 ...
 */
export function baseUnitsToString(base: bigint): string {
  if (base < 0n) throw new MoneyError("NEGATIVE", `base units must be non-negative, got ${base}`);
  return base.toString();
}

export function baseUnitsFromString(s: string): bigint {
...
export function assertFitsUint256(base: bigint, what = "amount"): void {
  if (base < 0n) throw new MoneyError("NEGATIVE", `${what} must be non-negative, got ${base}`);
  if (base > MAX_UINT256) {
