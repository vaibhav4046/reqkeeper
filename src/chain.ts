...
export async function rpcCall(
  ...
  for (const alt of rpcFallbacks()) {
    if (sameEndpoint(alt, rpcUrl)) continue;
    try {
...
function isOursAndWrong(kinds: readonly ConflictKind[]): boolean {
...
async function corroborate(
  ...
  for (const alt of rpcFallbacks()) {
    if (sameEndpoint(alt, primaryUrl)) continue;
    try {
      await ensureChain(alt);
...
export async function readReceipt(
  ...
  if (r.blockNumber !== undefined) {
    try {
      const block = (await rpcCall(rpcUrl, "eth_getBlockByNumber", [r.blockNumber, false], timeoutMs)) as { timestamp?: string } | null;
      if (block?.timestamp) blockTimestamp = Number(BigInt(block.timestamp));
    } catch {
      // unknown, not zero
    }
  }
  const common = { hash, gasUsed, to: r.to, logs: r.logs, blockNumber, blockTimestamp, confirmations, source: "chain" as const };
