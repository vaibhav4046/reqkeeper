...
const SUPPORTED = new Set<string>(["address", "uint256", "bytes"]);
...
export function parseSignature(signature: string): { name: string; types: AbiType[] } {
  ...
  if (!m) throw new AbiError("BAD_SIGNATURE", `not a function signature: ${signature}`);
  ...
  return { name: m[1], types: types as AbiType[] };
}

function stripHex(v: string, code: AbiErrorCode, what: string): string {
...
function encodeAddress(v: unknown): string {
  ...
  if (hex.length !== 40) throw new AbiError("BAD_ADDRESS", `address must be 20 bytes, got ${hex.length / 2}`);
...
function encodeUint256(v: unknown): string {
  let n: bigint;
  if (typeof v === "bigint") n = v;
  else if (typeof v === "string" && /^\d+$/.test(v.trim())) n = BigInt(v.trim());
  else throw new AbiError("BAD_UINT", `uint256 must be a decimal string or bigint, got ${String(v)}`);
  if (n < 0n) throw new AbiError("BAD_UINT", "uint256 cannot be negative");
  if (n > UINT256_MAX) throw new AbiError("BAD_UINT", "value exceeds uint256");
  return n.toString(16).padStart(WORD, "0");
}

function encodeBytesTail(v: unknown): string {
  const hex = stripHex(String(v), "BAD_BYTES", "bytes");
  if (hex.length % 2 !== 0) throw new AbiError("BAD_BYTES", "bytes must be whole octets");
...
export function encodeCall(signature: string, args: readonly unknown[]): string {
...
export function decodeCall(signature: string, calldata: string): string[] {
  const { types } = parseSignature(signature);
  const hex = stripHex(calldata, "BAD_CALLDATA", "calldata");
  if (hex.length < 8) throw new AbiError("BAD_CALLDATA", "calldata shorter than a selector");
  ...
  const word = (i: number): string => body.slice(i * WORD, (i + 1) * WORD);
  const out: string[] = [];
  ...
  for (let i = 0; i < types.length; i++) {
    ...
    switch (types[i]) {
      case "address": {
        if (!/^0{24}/.test(w)) throw new AbiError("BAD_CALLDATA", `address word ${i} has dirty high bytes`);
      ...
      case "bytes": {
        ...
        if (at + WORD > body.length) throw new AbiError("BAD_CALLDATA", `bytes offset ${offsetBytes} out of range`);
        const len = Number(BigInt(`0x${body.slice(at, at + WORD)}`));
        const start = at + WORD;
        if (start + len * 2 > body.length) throw new AbiError("BAD_CALLDATA", `bytes length ${len} out of range`);
 ...
 */
export function decodeAndVerify(signature: string, calldata: string): string[] {
  ...
  if (reencoded.toLowerCase() !== calldata.toLowerCase()) {
    throw new AbiError(
      "BAD_CALLDATA",
      `re-encoding did not reproduce the original calldata\n  original:  ${calldata}\n  re-encoded: ${reencoded}`,
    );
  }
  return args;
}
