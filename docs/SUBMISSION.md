...
4. **KeeperHub executes.** Through two surfaces behind one interface, both proven with money:
   the REST direct-execution API (38 settlements, `docs/refusals-live.json`) and KeeperHub's own
   MCP server (3 settlements, `docs/evidence/mcp-settlements.json`, each carrying its KeeperHub
   execution id and its transaction). Both pass the same calldata gate.
