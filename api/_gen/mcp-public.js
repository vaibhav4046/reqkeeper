    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                ...
                expect: {
                    type: "object",
                    description: "The payment you believe is owed. Without it a sighting is reported, never a payment.",
                    properties: {
...
export const RETRY_GUIDANCE = {
    REFERENCE_MISMATCH: "The payment reference supplied does not match the one this invoice derives. That is a " +
        "different debt, or a forged one: do not retry with the same value.",
    FACTS_DISAGREE_WITH_INVOICE: "What was supplied disagrees with what Request serves for this invoice. Read the invoice " +
        "again and propose from it; retrying the same facts is refused the same way.",
    REQUEST_UNREADABLE: "Request's gateway could not be read, so nothing is asserted about the invoice either way. " +
    ...
    SETTLED: "The receipt and the fee-proxy event for that same transaction both check out, read here " +
    ...
    SOURCE_UNVERIFIABLE: "The chain could not be read far enough to say whether the invoice is already paid. " +
    ...
    EXECUTION_REVERTED: "The transaction reverted on chain. Nothing moved, and this obligation is terminal: it " +
...
function readExpectation(raw) {
    ...
    if (!HEX_ADDRESS.test(tokenAddress))
        throw new Error("expect.tokenAddress must be a 0x-prefixed 20-byte address");
    if (!HEX_ADDRESS.test(to))
        throw new Error("expect.to must be a 0x-prefixed 20-byte address");
    if (!BASE_UNITS.test(amount))
        throw new Error("expect.amount must be base units as a decimal string");
